...
public class UnawareDividedCompactionWorkerOperator
    ...
    Iterable<Future<CommitMessage>> result() {
        return compactionHelper.result();
    }

    @Override
    public void open() throws Exception {
        LOG.debug("Opened a append-only table compaction worker.");
        this.compactionHelper =
                new UnwareBucketCompactionHelper(table, commitUser, this::workerExecutor);
    }

    @Override
    protected List<Committable> prepareCommit(boolean waitCompaction, long checkpointId)
            throws IOException {
        return this.compactionHelper.prepareCommit(waitCompaction, checkpointId);
    }

    @Override
    public void processElement(StreamRecord<AppendOnlyCompactionTask> element) throws Exception {
        this.compactionHelper.processElement(element);
    }
